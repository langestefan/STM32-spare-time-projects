...
wavSound * loadWaveHeader(FILE * fp) {
	...
	printf( "Samplerate         = %d\n", w->sampleRate );
	printf( "Number of channels = %d", w->numChannels );
	if ( w->numChannels == 1 ) printf( " (Mono)");
	if ( w->numChannels == 2 ) printf( " (Sterio)");
	printf( "\nBits per sample    = %d\n", w->bitsPerSample );
	printf( "Data lenght        = %d\n", w->dataLength );

	if ( w->sampleRate != 8000 )
        printf( "Please use sample rate of 8kbs!\n");

	if ( w->numChannels != 1 )
        printf( "Please use mono wav file!\n");

	printf( "Duration           = %.2f sec\n", (float)w->dataLength / w->sampleRate );
	return w;
}
...
void saveWave_(FILE * fpI, wavSound *s, FILE * fpO, char * name, int MaxSamples) {
    ...
    // truncate number of bytes to multiples of 8 ??
    ...
    {
        fprintf(fpO, "// 8 bits per sample\n\n");
		fprintf(fpO, "const unsigned char %s_data[] ={", name);
		if (s->bitsPerSample == 8)
        {
			for (i = 0; i < realLength; i++) {
                // write lines of 20 values with the index at the left side
				if ( (i % 16) == 0 )
                {
                    fprintf(fpO, "\n/* %4d */ ", i);
				}
                if ((stuff8 = fgetc(fpI)) == EOF )
                {
                    printf("Error when reading wav file: EOF");
                    printf( " after reading %d bytes\n", i );
                    exit(EXIT_FAILURE);
                }
				fprintf(fpO, "%3d, ", (unsigned char) stuff8);
			}
			fprintf(fpO, "};\n");
		} else {
            fprintf(fpO, "// 16 bits per sample\n\n");
			for (i = 0 ; i < realLength ; i++) {
				fread(&stuff8, sizeof(char), 1, fpI);
				fread(&stuff8, sizeof(char), 1, fpI);
				fprintf(fpO, "%d, ", (signed char)stuff8);
